...
/* --------------------------------------------------
   HYBRID MODEL (RUGBY UNION OPTIMIZED)
-------------------------------------------------- */
...
function mapCompetitionToSportKey(code) {
  ...
  if (c === "TOP14") return "rugbyunion_top14";
  if (c === "SIXNATIONS") return "rugbyunion_six_nations";

  return "rugbyunion_top14";
}
...
async function safeRequest(url, params) {
  try {
    ...
    console.error("Rugby Union API error:", error.response?.data || error.message);
    return null;
  }
}
...
/* --------------------------------------------------
   MAIN FUNCTION
-------------------------------------------------- */
async function getRugbyUnionTopPicks(competition) {

  const cacheKey = `rugby-union-${competition}`;
