...
/* ==========================================================
   ...
   to:
   "us,uk,eu"
   This improves bookmaker availability globally.
========================================================== */
...
function mapCompetitionToSportKey(competition) {
  ...
  if (comp === "nfl") return "americanfootball_nfl";
...
async function safeRequest(url, params) {
  try {
    ...
    console.error("NFL API error:", error.response?.data || error.message);
    return null;
  }
}
...
/* --------------------------------------------------
   MAIN FUNCTION
-------------------------------------------------- */
async function getNFLTopPicks(competition) {

  const cacheKey = `nfl-${competition}`;
