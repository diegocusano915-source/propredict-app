...
/* --------------------------------------------------
   HYBRID MODEL (RUGBY LEAGUE OPTIMIZED)
-------------------------------------------------- */
...
function mapCompetitionToSportKey(competition) {
  ...
  if (comp === "nrl") return "rugbyleague_nrl";

  return "rugbyleague_nrl";
}
...
async function safeRequest(url, params) {
  try {
    ...
    console.error("Rugby League API error:", error.response?.data || error.message);
    return null;
  }
}
...
/* --------------------------------------------------
   MAIN FUNCTION
-------------------------------------------------- */
async function getRugbyLeagueTopPicks(competition) {

  const cacheKey = `rugby-league-${competition}`;
