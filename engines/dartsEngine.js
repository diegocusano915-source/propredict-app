...
function mapCompetitionToSportKey(competition) {
  ...
  if (comp === "pdc") return "darts";
  if (comp === "bdo") return "darts";
...
async function safeRequest(url, params) {
  try {
    ...
    console.error("Darts API error:", error.response?.data || error.message);
    return null;
  }
}
...
/* --------------------------------------------------
   MAIN FUNCTION
-------------------------------------------------- */
async function getDartsTopPicks(competition) {

  const cacheKey = `darts-${competition}`;
  const now = Date.now();
  ...
  if (coreData) {
    extractMarkets(coreData, groupedMarkets);
  }
