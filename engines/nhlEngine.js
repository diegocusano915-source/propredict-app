...
function calculateHybridProbability(impliedProbability, marketKey) {
  ...
  if (marketKey === "spreads") modelAdjustment = 1.5; // puck line
...
function mapCompetitionToSportKey(competition) {
  ...
  if (comp === "nhl") return "icehockey_nhl";
...
async function safeRequest(url, params) {
  try {
    ...
    console.error("NHL API error:", error.response?.data || error.message);
    return null;
  }
}
...
/* --------------------------------------------------
   MAIN FUNCTION
-------------------------------------------------- */
async function getNHLTopPicks(competition) {

  const cacheKey = `nhl-${competition}`;
