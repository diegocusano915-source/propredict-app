...
/* ==========================================================
   TENNIS ENGINE (ATP + WTA)
   ----------------------------------------------------------
   Adjusted to match available sport keys in free plan:
...
function calculateHybridProbability(impliedProbability, marketKey) {
  ...
  if (marketKey === "spreads") modelAdjustment = 1;
  if (marketKey === "totals") modelAdjustment = 1;

  const hybrid =
    (impliedProbability * 0.85) +
    ((impliedProbability + modelAdjustment) * 0.15);
...
function mapCompetitionToSportKey(code) {
  ...
  // ✅ Map to actual available tournaments
  if (c === "ATP") return "tennis_atp_indian_wells";
  if (c === "WTA") return "tennis_wta_indian_wells";

  // Default fallback
  return "tennis_atp_indian_wells";
}
...
/* --------------------------------------------------
   MAIN FUNCTION
-------------------------------------------------- */
async function getTennisTopPicks(competition) {

  const cacheKey = `tennis-${competition}`;
