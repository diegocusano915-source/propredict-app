...
/* --------------------------------------------------
   ✅ DERIVED EFFICIENCY MODEL (MARKET-BASED)
   Uses spread + total to estimate scoring strength
-------------------------------------------------- */
function applyDerivedEfficiencyAdjustment(
  hybridProbability,
  impliedScore,
  opponentImpliedScore
) {

  const netScoreEdge = impliedScore - opponentImpliedScore;

  let adjustment = 0;

  if (netScoreEdge >= 10) adjustment = 2;
  else if (netScoreEdge >= 6) adjustment = 1;
  else if (netScoreEdge <= -6) adjustment = -1;
...
/* --------------------------------------------------
   EXTRACT MARKETS WITH HYBRID + EFFICIENCY LOGIC
-------------------------------------------------- */
function extractMarkets(data, groupedMarkets) {
  ...
  for (const game of data.slice(0, 5)) {
    ...
    // ✅ Identify spread & totals for derived model
      ...
      if (market.key === "h2h" && hybridValues.length === 2) {
        ...
        if (spreadMarket && totalsMarket) {
          ...
          const impliedScoreFirst =
            (totalPoint / 2) - spreadFirst;

          const impliedScoreSecond =
            (totalPoint / 2) - spreadSecond;
