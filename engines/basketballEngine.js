  ...
  for (const game of data.slice(0, 5)) {
    ...
    for (const market of bookmaker.markets) {
      ...
      if (market.key === "h2h" && hybridValues.length === 2) {
        ...
        // ✅ Conservative Derived Efficiency
        if (spreadMarket && totalsMarket) {
          ...
          // ✅ Correct implied scoring formula
        ...
        // ✅ Final safety clamp to prevent inflation stacking
        first.hybrid = Math.min(first.hybrid, 85);
        second.hybrid = Math.min(second.hybrid, 85);
      }
