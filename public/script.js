...
async function loadTopPicks(league) {
    ...
    try {
        ...
        const picks = Array.isArray(data?.topPicks)
            ? data.topPicks
            : [];

        topPicksData = picks.map(pick => {

            const matchString = pick.match || "";
            const parts = matchString.split(" vs ");
            const home = parts[0] || "Unknown";
            const away = parts[1] || "Unknown";
