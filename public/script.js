...
// Stable Build With Team Analysis Restored
// ==========================================================
...
async function loadTeamsForLeague(leagueCode) {
    ...
    DOM.teamSelect.innerHTML = `<option value="">Loading teams...</option>`;

    try {

        const response = await fetch(`/api/league-teams/${leagueCode}`);
    ...
    if (DOM.combinedOdds) {
        ...
        if (combinedProbability > 0) {
            DOM.combinedOdds.textContent =
                (1 / combinedProbability).toFixed(2);
        } else {
            DOM.combinedOdds.textContent = "-";
        }
...
function renderTeamAnalysis(data) {
    ...
    if (DOM.fullTimeGrid) {
        DOM.fullTimeGrid.innerHTML = buildAnalysisGrid(data.fullTime);
    }

    if (DOM.firstHalfGrid) {
        DOM.firstHalfGrid.innerHTML = buildAnalysisGrid(data.firstHalf);
    }

}


function buildAnalysisGrid(section) {
