...
function isSelectionAdded(id) {
    return builderSelections.find(item => item.id === id);
}
...
function addToBuilder(matchName, market, probability, buttonElement) {
    const id = `${matchName}-${market}`;

    if (isSelectionAdded(id)) return;
    ...
    buttonElement.classList.add("added-btn");
...
function removeFromBuilder(id) {
    ...
    const btn = document.querySelector(`[data-selection-id="${id}"]`);
    if (btn) {
        btn.textContent = "Add";
        btn.disabled = false;
...
function renderBuilder() {
    ...
    if (combinedProb > 0) {
        combinedOddsEl.textContent = (100 / combinedProb).toFixed(2);
...
clearBuilderBtn.addEventListener("click", () => {
    builderSelections.forEach(item => {
        const btn = document.querySelector(`[data-selection-id="${item.id}"]`);
        if (btn) {
            btn.textContent = "Add";
            btn.disabled = false;
            btn.classList.remove("added-btn");
    ...
    try {
        ...
        data.topPicks.forEach(match => {
            ...
            markets.forEach(item => {
                ...
                const id = `${match.match}-${item.label}`;

                const addBtn = document.createElement("button");
                addBtn.textContent = isSelectionAdded(id) ? "Added" : "Add";
                addBtn.disabled = isSelectionAdded(id);
                addBtn.className = "add-selection-btn";
                addBtn.dataset.selectionId = id;

                if (isSelectionAdded(id)) {
                    addBtn.classList.add("added-btn");
                }

                addBtn.onclick = () =>
                    addToBuilder(match.match, item.label, item.value, addBtn);
