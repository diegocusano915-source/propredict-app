...
(function () {
  ...
  function checkDataState() {
    ...
    setTimeout(() => {
      ...
      if (!hasData) {
        ...
        renderEmptyState(
          topPicksContainer,
          "No match data available (API limit reached or unavailable)"
        );

        renderEmptyState(
          teamAnalysisContainer,
          "Team analysis unavailable due to API limit"
        );

        renderEmptyState(
          marketOptionsContainer,
          "Betting markets unavailable at the moment"
        );
  ...
  // ==========================================================
  // NEW: PROFESSIONAL EMPTY STATE OVERLAY (ADDED)
  // ==========================================================

  function enhanceEmptyStates() {

    const blocks = document.querySelectorAll(".panel-content-grid");

    blocks.forEach(block => {

      if (!block.children.length || block.innerText.includes("No match data")) {
        ...
        block.innerHTML = `
          ...
          ">
            ...
            <div style="
              ...
              line-height: 1.5;
            ">
              Live match data could not be loaded due to API limits.<br>
              Please try again later or upgrade access for higher limits.
            </div>
          </div>
        `;
      }
  ...
  document.addEventListener("DOMContentLoaded", function () {
    ...
    // NEW ADD-ON RUN
    setTimeout(enhanceEmptyStates, 1800);
