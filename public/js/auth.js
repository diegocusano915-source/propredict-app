...
// 🔥 ADDED: Check if user is Pro (or VVIP/Admin)
window.ppIsPro = function ppIsPro() {
  const role = (window.ppAuthState.role || "free").toLowerCase();
  return role === "pro" || role === "vvip" || role === "admin";
};

// 🔥 ADDED: Get current user role
window.ppGetUserRole = function ppGetUserRole() {
  return window.ppAuthState.role || "free";
};

// 🔥 ADDED: Get current user email from token
window.ppGetCurrentUser = function ppGetCurrentUser() {
  if (!window.ppAuthState.payload) return null;
  return {
    email: window.ppAuthState.payload.email,
    role: window.ppAuthState.payload.role || "free",
    id: window.ppAuthState.payload.id
  };
};
...
window.ppApplyRole = function ppApplyRole(role) {
  ...
  // 🔥 ADDED: Trigger pro locks refresh
  if (typeof window.refreshProLocks === 'function') {
    setTimeout(window.refreshProLocks, 50);
  }
};
...
window.ppSyncProtectedUI = function ppSyncProtectedUI() {
  ...
  // 🔥 ADDED: Refresh pro locks when UI syncs
  if (typeof window.refreshProLocks === 'function') {
    setTimeout(window.refreshProLocks, 50);
  }
};
