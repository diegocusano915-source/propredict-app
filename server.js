...
const { getMLBTopPicks } = require("./engines/mlbEngine"); // ✅ MLB ADDED
...
app.get("/api/elite/:sport/:competition", async (req, res) => {
  ...
  try {
    ...
    if (sport === "football") {
      picks = await getFootballOddsTopPicks(competition);
    }
    ...
    if (sport === "nfl") {
      picks = await getNFLTopPicks(competition);
    }

    if (sport === "nhl") {
      picks = await getNHLTopPicks(competition);
    }

    if (sport === "rugbyleague") {
      picks = await getRugbyLeagueTopPicks(competition);
    }

    if (sport === "rugbyunion") {
      picks = await getRugbyUnionTopPicks(competition);
    }

    // ✅ MLB ADDED
    if (sport === "mlb") {
      picks = await getMLBTopPicks(competition);
    }
...
app.post("/api/accumulator/smart", async (req, res) => {
  try {
    ...
    if (sport === "football") {
      markets = await getFootballOddsTopPicks(competition);
    }

    if (sport === "basketball") {
      markets = await getBasketballTopPicks(competition);
    }

    if (sport === "nfl") {
      markets = await getNFLTopPicks(competition);
    }

    if (sport === "nhl") {
      markets = await getNHLTopPicks(competition);
    }

    if (sport === "rugbyleague") {
      markets = await getRugbyLeagueTopPicks(competition);
    }

    if (sport === "rugbyunion") {
      markets = await getRugbyUnionTopPicks(competition);
    }

    // ✅ MLB ADDED
    if (sport === "mlb") {
      markets = await getMLBTopPicks(competition);
    }
    ...
    const result = generateSmartAccumulator(markets, {
      targetMinOdds: targetMinOdds || 10,
      targetMaxOdds: targetMaxOdds || 20,
