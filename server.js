  ...
  contentSecurityPolicy: {
    directives: {
      ...
      styleSrc: [
        ...
        "https://*.tawk.to",
        "https://cdn.jsdelivr.net",
        "https://cdnjs.cloudflare.com" // ADDED: Fixes Font Awesome
      ],
      fontSrc: [
        "'self'", 
        "data:", 
      ...
      connectSrc: [
        ...
        "https://v1.basketball.api-sports.io",        "https://v1.american-football.api-sports.io",
...
async function getGeoLocation(ip) {
  ...
  try {
    ...
    if (response.data.status === "success") {
      ...
      geoCache.set(ip, { data: geoData, timestamp: Date.now() });      return geoData;
    }
...
async function initializeAuthTables() {
  try {
    ...
    // ADDED: Prevents hidden 500 errors when granting pro access
...
app.post("/api/auth/google", async (req, res) => {
  ...
  } catch (e) {    console.error("[auth] google sign-in failed:", e.response?.data?.message || e.message);
...
app.get("/api/user/referral-stats", authenticateToken, async (req, res) => {  try {
...
app.post("/api/auth/forgot-password", async (req, res) => {
  try {
    ...
    // Replaced fetch with axios and added error logging
    await axios.post(
  ...
  } catch (error) {     console.error("[Auth] Forgot password failed:", error.response?.data || error.message);
...
app.get("/api/league-teams/:leagueCode", async (req, res) => {
  try {
    ...
    res.json(response.teams.map(t => ({ id: t.id, name: t.name })));  } catch (e) { res.json([{ id: 57, name: "Arsenal" }, { id: 65, name: "Manchester City" }]); }
});
...
async function botGetEvents() {
  ...
  const leagues = [
    { name: "Premier League", id: "4328" }, { name: "Spanish La Liga", id: "4335" },    { name: "Italian Serie A", id: "4332" }, { name: "German Bundesliga", id: "4331" },
...
async function botFridayPreview() {
  ...
  for (const e of upcoming) {
    ...
    const text = await botGenerateText(prompt, 800);    if (text) await botPostPrediction(`${home} vs ${away}`, text, "Football", league, Math.floor(Math.random()*2)+4, (Math.random()*1.3+1.6).toFixed(2), date);
...
app.get("/api/bot/run-now", async (req, res) => {
  ...
  // ADDED: catch to prevent background crash
  botFridayPreview().catch(err => console.error("Bot manual run failed:", err));
});

setInterval(() => { const today = new Date().toLocaleString("en-US", { weekday: "long", timeZone: "UTC" }); if (["Monday","Wednesday","Friday"].includes(today)) botFridayPreview().catch(err => console.error("Bot scheduled run failed:", err)); }, 6 * 60 * 60 * 1000);
setTimeout(() => botFridayPreview().catch(err => console.error("Bot initial run failed:", err)), 30000);
...
app.post("/telegram-webhook", async (req, res) => {
  try {
    ...
    if (text === "/START") {
      ...
      const sportKey = text.split(" ")[1] || "PL";      const result = await getPicksForSport(sportKey, false);
...
app.post("/api/paystack/initialize-subscription", authenticateToken, async (req, res) => {
  try {
    ...
    if (!subResp.data?.status || !subResp.data.data?.authorization_url) {      console.error("[payments] Paystack rejected initialization:", JSON.stringify(subResp.data));
...
app.post("/api/get-payment-details", authenticateToken, async (req, res) => {
  try {
    ...
    } else {      details = { ...details, instructions: "This currency is not currently supported for manual payment. Contact support." };
    }
...
const {
  ...
  getRecentArticles,
  getActiveLeagues,
  loadMeta} = require("./services/newsStorage");
