...
/* --------------------------------------------------
   TELEGRAM WEBHOOK ROUTE (INTERACTIVE + MANUAL)
-------------------------------------------------- */
    ...
    if (body.callback_query) {
      ...
      if (data === "sport_football") {
        ...
        const keyboard = {
          inline_keyboard: [
            [
              { text: "PL", callback_data: "football_PL" },
              { text: "PD", callback_data: "football_PD" },
              { text: "SA", callback_data: "football_SA" }
            ],
            [
              { text: "BL1", callback_data: "football_BL1" },
              { text: "FL1", callback_data: "football_FL1" },
              { text: "EFL", callback_data: "football_EFL" }
            ]
          ]
        };

        await sendTelegramMessage("*Select League:*", keyboard);
      }
      ...
      if (data === "sport_nfl") {
        ...
        const picks = await getNFLTopPicks("NFL");
        const top = picks.h2h.slice(0, 3);

        let reply = `*NFL Top Picks*\n\n`;
      ...
      /* ----- TENNIS ----- */
      if (data === "sport_tennis") {

        const picks = await getTennisTopPicks("ATP");
        const top = picks.h2h.slice(0, 3);

        let reply = `*Tennis (ATP) Top Picks*\n\n`;
