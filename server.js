...
/* ==================================================
   ================= LOGIN ROUTE (UPDATED) ===========
   ====== Hardcoded bot login added below ============
================================================== */
app.post("/api/auth/login", async (req, res) => {
  try {
    const { email, password, rememberMe } = req.body;

    // ========== BOT / HARDCODED ADMIN LOGIN ==========
...
async function botGenerateText(prompt) {
  try {
...
async function botGetUpcomingEvents() {
  ...
  for (const league of leagues) {
    try {
      ...
      console.log(`⚠️ Bot: Failed to fetch ${league.name}`);
    }
  }
  return allEvents;
}
...
async function botGetRecentResults() {
  ...
  for (const league of leagues) {
    try {
      ...
      console.log(`⚠️ Bot: Failed to fetch results for ${league.name}`);
    }
  }
  return results;
}
...
async function botPostPrediction(matchName, predictionText, sport, league, confidence, odds, matchDate) {
  try {
    const token = jwt.sign(
      { id: "admin-bot", email: ADMIN_EMAIL, role: "admin" },
      JWT_SECRET,
      { expiresIn: "1h" }
    );
...
async function botMondayRecap() {
  ...
  for (const event of selected) {
    ...
    const prompt = `Write a short, punchy football recap (2-3 sentences) about this real match result:

MATCH: ${home} ${homeScore} - ${awayScore} ${away}
LEAGUE: ${league}
DATE: ${date}

Tone: Sports analyst, human-sounding, no AI fluff. Be direct like a football pundit. Mention the score and one key implication. Plain English, no markdown.`;
    
    const recap = await botGenerateText(prompt);
    const finalRecap = recap || `${home} defeated ${away} ${homeScore}-${awayScore} in an entertaining ${league} clash. The result keeps ${home} on track while ${away} will need to regroup.`;
...
async function botWednesdayPreview() {
  ...
  for (const event of selected) {
    ...
    const prompt = `Write a short match preview (3-4 sentences) for this real upcoming fixture:

MATCH: ${home} vs ${away}
LEAGUE: ${league}
DATE: ${date} at ${timeStr}

Tone: Confident betting analyst. Include a brief prediction and one reason why. Sound human, like a tipster. No markdown.`;
    
    const preview = await botGenerateText(prompt);
    let predictionLabel = "Home Win";
    const finalPreview = preview || `${home} host ${away} in a competitive ${league} encounter. ${home} enter as slight favorites but ${away} have the quality to cause problems.`;
    
    if (preview && (preview.toLowerCase().includes("draw") || preview.toLowerCase().includes("stalemate"))) {
...
async function botFridayPreview() {
  console.log("\n🤖 BOT: Friday Weekend Big Preview");
  const events = await botGetUpcomingEvents();
  if (events.length < 2) {
    console.log("⚠️ Not enough events, skipping");
    return;
  ...
  for (const event of selected) {
    ...
    const prompt = `Write a detailed betting-focused match preview (4-5 sentences) for this real fixture:

MATCH: ${home} vs ${away}
LEAGUE: ${league}
DATE: ${date} at ${timeStr}

Include: team form hint, a specific betting tip, and a predicted scoreline. Tone: Expert tipster, confident but realistic. Human, conversational. No markdown.`;
    
    const preview = await botGenerateText(prompt);
    const finalPreview = preview || `All eyes on this ${league} clash as ${home} welcome ${away}. We're leaning toward goals here. The value bet looks to be Both Teams to Score. Score prediction: 2-1.`;
...
// Schedule: Check every 6 hours if it's Mon/Wed/Fri
setInterval(botRunScheduled, 6 * 60 * 60 * 1000);

// Run once on startup
setTimeout(botRunScheduled, 10000);
