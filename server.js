...
/* --------------------------------------------------
   SAFE AXIOS WRAPPER (NEW — 429 PROTECTION)
-------------------------------------------------- */
...
/* --------------------------------------------------
   TOP PICKS (429 RESILIENT + CACHE SAFE)
-------------------------------------------------- */
app.get("/api/top-picks/:leagueCode", async (req, res) => {
  ...
  try {
    ...
    // ✅ If rate limited → fallback to cache
    ...
    // ✅ Save to cache even if empty (prevents hammering API)
    cache[cacheKey] = {
      data: finalTopPicks,
      timestamp: now
    };
