...
/* --------------------------------------------------
   GET TEAMS BY LEAGUE (ORIGINAL ROUTE)
-------------------------------------------------- */
...
/* --------------------------------------------------
   LEAGUE TEAMS ALIAS (FOR FRONTEND COMPATIBILITY)
-------------------------------------------------- */
...
app.get("/api/top-picks/:leagueCode", async (req, res) => {
  ...
  try {
    ...
    // ✅ Fetch scheduled matches safely
    ...
    // ✅ If rate limited → return cached safely
    ...
    // ✅ Save cache even if empty (prevents API hammering)
    cache[cacheKey] = {
      data: finalTopPicks,
      timestamp: now
    };
