...
/* ===========================
   REFERRAL CONFIG
=========================== */
const REFERRAL_REWARD_DAYS = 7; // Both users get 7 free days of Pro access
...
app.post("/api/auth/register", async (req, res) => {
  try {
    ...
    // ✅ If new user was referred, grant them Pro access
    if (referredBy) {
      await grantProAccess(supabaseUserId, REFERRAL_REWARD_DAYS);
      // The referrer reward will be claimed after email verification
    }
...
// ✅ Webhook for email verification (called by Supabase)
app.post("/api/auth/email-verified", async (req, res) => {
  try {
    ...
    // If user was referred, claim reward for referrer
    if (user.referred_by) {
...
// ✅ Apply referral code (for logged in users upgrading)
