  ...
  try {
    ...
    if (body.message) {
      ...
      if (text === "/start") {
        const keyboard = {
          inline_keyboard: [
            [{ text: "📊 Today's Picks", callback_data: "menu_sports" }]
          ]
        };
        await sendTelegramMessage("*🏆 Welcome to ProPredict Bot!*\n\nGet daily AI-powered sports predictions.\n\nTap below to see today's picks:", keyboard);
      }
      
      if (text === "/picks" || text === "/today") {
        try {
          const picks = await getFootballOddsTopPicks("PL");
          const top = picks.h2h.slice(0, 5);
          let message = `*⚽ Today's Premier League Picks*\n\n`;
          top.forEach((p, i) => {
            message += `${i+1}. *${p.market}*\n   📊 ${p.adjustedProbability}% confidence (${p.confidence})\n\n`;
          });
          await sendTelegramMessage(message);
        } catch (e) {
          await sendTelegramMessage("❌ Unable to fetch picks right now. Please try again later.");
        }
      }
      return res.sendStatus(200);
    }
...
function scheduleDailyTelegramPicks() {
  // Send immediately on server start (for testing)
  setTimeout(async () => {
    try {
      await sendTelegramMessage("🚀 *ProPredict Bot Started*\n\nDaily picks will arrive every 6 hours.");
    } catch (e) {
  ...
  setInterval(async () => {
    try {
      const picks = await getFootballOddsTopPicks("PL");
      const top = picks.h2h.slice(0, 5);
      let message = `*⚽ Premier League Top Picks*\n_${new Date().toLocaleDateString()}_\n\n`;
      top.forEach((p, i) => {
...
console.log("✅ Telegram: /api/test-telegram endpoint ready");
