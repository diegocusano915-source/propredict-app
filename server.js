  ...
  try {
    ...
    await pool.query(
      `INSERT INTO users (id, email, password_hash, role)
       VALUES ($1, $2, $3, $4)`,
      [supabaseUserId, email, hashedPassword, role]
    );
...
/* ==================================================
   ...
   ===== CHAT ID: 6816699294 =======================
   ===== ENHANCED: On-demand picks by sport =========
================================================== */
...
// Sport to competition mapping for commands
...
async function getPicksForSport(sportKey) {
...
function formatPicksMessage(config, picks, limit = 5) {
  const top = picks.h2h?.slice(0, limit) || [];
  ...
  top.forEach((p, i) => {
    ...
    message += `   📊 ${p.adjustedProbability}% confidence (${p.confidence})\n`;
...
app.post("/telegram-webhook", async (req, res) => {
  try {
      ...
      if (data === "menu_sports") {
        const keyboard = {
          inline_keyboard: [
            ...
            [
              { text: "🎾 Tennis", callback_data: "picks_TENNIS" },
              { text: "🏒 NHL", callback_data: "picks_NHL" }
            ]
          ]
        };
        await sendTelegramMessage("*Select Sport for Picks:*", keyboard);
      } else if (data.startsWith("picks_")) {
        const sportKey = data.replace("picks_", "");
        const result = await getPicksForSport(sportKey);
        
        if (result) {
          const message = formatPicksMessage(result.config, result.picks);
          if (message) {
    ...
    if (body.message) {
      ...
      // /help command
      if (text === "/HELP") {
        await sendTelegramMessage("*📚 ProPredict Bot Commands*\n\n`/start` - Welcome message\n`/picks [SPORT]` - Get picks\n`/help` - Show this help\n\n*Available Sports:*\nPL, UCL, UEL, PD, SA, BL1, NFL, NBA, TENNIS, NHL, MLB\n\nExample: `/picks UCL`");
        return res.sendStatus(200);
      }
...
function scheduleDailyTelegramPicks() {
  // Send immediately on server start
  setTimeout(async () => {
    try {
  ...
  // Schedule every 6 hours - send Premier League picks
  const SIX_HOURS = 6 * 60 * 60 * 1000;
  setInterval(async () => {
    try {
      const result = await getPicksForSport('PL');
      if (result) {
        const message = formatPicksMessage(result.config, result.picks);
        if (message) {
          await sendTelegramMessage(message);
...
console.log("✅ Admin routes loaded (AUTH TEMPORARILY BYPASSED)");
console.log("✅ Forgot Password route added at /api/auth/forgot-password");
...
console.log("✅ Telegram: /picks command enabled for PL, UCL, NFL, NBA, TENNIS, NHL, MLB");
