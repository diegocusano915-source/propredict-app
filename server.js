...
/* ===========================
   SECURITY MIDDLEWARE — FINAL CSP WITH DATA: FOR FONTS
=========================== */
app.use(helmet({
  contentSecurityPolicy: {
    directives: {
      defaultSrc: ["'self'"],
      scriptSrc: ["'self'", "'unsafe-inline'", "'unsafe-hashes'", "https://js.paystack.co", "https://propredict-app.onrender.com", "https://embed.tawk.to", "https://tawk.to", "https://va.tawk.to"],
      scriptSrcAttr: ["'self'", "'unsafe-inline'", "'unsafe-hashes'"],
      styleSrc: ["'self'", "'unsafe-inline'", "https://fonts.googleapis.com", "https://embed.tawk.to"],
      fontSrc: ["'self'", "data:", "https://fonts.gstatic.com"],
      imgSrc: ["'self'", "data:", "https:"],
      connectSrc: ["'self'", "https://api.paystack.co", "https://propredict-app.onrender.com", "https://fonts.googleapis.com", "https://fonts.gstatic.com", "https://embed.tawk.to", "https://tawk.to", "https://va.tawk.to", "wss://embed.tawk.to", "wss://va.tawk.to", "http://ip-api.com"],
      frameSrc: ["'self'", "https://checkout.paystack.com", "https://embed.tawk.to", "https://tawk.to", "https://va.tawk.to"],
    },
  },
}));
...
app.post("/api/auth/register", async (req, res) => {
  try {
    ...
    const { data: authData, error: authError } = await supabaseAdmin.auth.admin.createUser({
      email: email,
      password: password,
      email_confirm: true,  // ✅ TURNED ON — sends verification email
    ...
    const welcomeHtml = `
      <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
        <h1 style="color: #00f5ff;">Welcome to ProPredict! 🎉</h1>
        <p>Hi ${email.split('@')[0]},</p>
        <p>Your account has been created successfully.</p>
        <p><strong>Please check your inbox for a verification email from Supabase</strong> to confirm your email address.</p>
        <p>Once verified, you'll have full access to ProPredict's AI-powered sports predictions.</p>
        <br>
        <p>Your referral code: <strong>${newReferralCode}</strong></p>
        <p>Share it with friends and you both get 7 days of Pro access FREE!</p>
        <br>
        <p>— The ProPredict Team</p>
      </div>
    `;
  ...
  try {
    ...
    const adminHtml = `
      <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
        ...
        <br>
        <p><a href="mailto:${email}" style="background: #00f5ff; color: #000; padding: 10px 20px; text-decoration: none; border-radius: 8px;">Reply to ${name}</a></p>
      </div>
    `;
    ...
    const userHtml = `
      <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
        <h1 style="color: #00f5ff;">✅ Message Received!</h1>
        <p>Hi ${name},</p>
        <p>Thank you for contacting ProPredict. We've received your message regarding "<strong>${subject}</strong>".</p>
        ...
        <br>
        <p>Best regards,</p>
        <p><strong>The ProPredict Team</strong></p>
        <p style="color: #666; font-size: 12px;">propredict.support@gmail.com</p>
      </div>
    `;
...
console.log("✅ IP tracking middleware active with geolocation (ip-api.com)");
...
console.log("✅ Email Verification: ENABLED (Supabase + Resend welcome email)");
console.log("✅ Contact Form: Active with Resend email sending");
...
console.log("✅ CSP Updated: Full Tawk.to support including fonts (data:)");
console.log("✅ Google OAuth: REMOVED");
