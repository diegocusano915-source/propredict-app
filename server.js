...
/* --------------------------------------------------
   WEIGHTED FORM ENGINE (FULL EXPANDED)
-------------------------------------------------- */
...
/* --------------------------------------------------
   SAFE AXIOS WRAPPER (429 PROTECTION)
-------------------------------------------------- */
...
/* --------------------------------------------------
   LEAGUE TEAMS ALIAS (FRONTEND COMPATIBILITY)
-------------------------------------------------- */
app.get("/api/league-teams/:leagueCode", async (req, res) => {
  try {
    ...
    console.error("League teams alias error:", error.message);
    res.json([]);
  }
});
/* --------------------------------------------------
   TEAM ANALYSIS ROUTE (STRUCTURED FOR FRONTEND)
-------------------------------------------------- */
app.get("/api/team-analysis/:teamId", async (req, res) => {
  try {
    ...
    const stats = calculateWeightedStats(matches, teamId);

    const structured = {
      fullTime: {
        "Avg Goals Scored": stats.avgGoalsScored,
        "Avg Goals Conceded": stats.avgGoalsConceded,
        "Over 0.5": stats.over05,
        "Over 1.5": stats.over15,
        "Over 2.5": stats.over25,
        "Over 3.5": stats.over35,
        "BTTS": stats.btts,
        "Win %": stats.winPercentage,
        "Draw %": stats.drawPercentage,
        "Loss %": stats.lossPercentage,
        "Double Chance 1X": stats.doubleChance1X,
        "Double Chance X2": stats.doubleChanceX2
      },
      firstHalf: {
        "FH Over 0.5": stats.fhOver05,
        "FH Over 1.5": stats.fhOver15,
        "FH BTTS": stats.fhBtts,
        "FH Win %": stats.fhWinPercentage,
        "FH Draw %": stats.fhDrawPercentage,
        "FH Loss %": stats.fhLossPercentage
      }
    };
...
/* --------------------------------------------------
   TOP PICKS (FREE-TIER SAFE + 429 RESILIENT)
-------------------------------------------------- */
app.get("/api/top-picks/:leagueCode", async (req, res) => {
  ...
  try {
    ...
    const allFixtures = fixturesData.matches || [];
    const fixtures = allFixtures.slice(0, 4);
    ...
    for (const match of fixtures) {
      ...
      if (!homeRes || homeRes.rateLimited) continue;
      ...
      if (!awayRes || awayRes.rateLimited) continue;
      ...
      const projectedHomeWin =
        (parseFloat(homeStats.winPercentage) +
         parseFloat(awayStats.lossPercentage)) / 2;
    ...
    projections.sort((a, b) =>
      Math.max(b.over25, b.btts, b.homeWin) -
    ...
    console.error("Top picks error:", error.message);
