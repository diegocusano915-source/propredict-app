...
/* --------------------------------------------------
   SMART ACCUMULATOR TEST (GET)
-------------------------------------------------- */
app.get("/api/accumulator/smart-test", async (req, res) => {

  try {

    const markets = await getFootballOddsTopPicks("PL");
    ...
    console.error("Smart test error:", error.message);
    res.json({
      selections: [],
      combinedProbability: 0,
      decimalOdds: 0,
